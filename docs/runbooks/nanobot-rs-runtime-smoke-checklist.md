# nanobot-rs Runtime Smoke Checklist
...
- Config lives in `~/.nanobot-rs/config.json` by default. `config.toml` is also supported if it already exists.
- Workspace lives in `~/.nanobot-rs/workspace` by default.
...
cargo run --release -- agent \
  --config /tmp/nanobot-codex-smoke.json \
  --session cli:smoke \
