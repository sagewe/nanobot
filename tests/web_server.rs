...
use sidekick::config::{AgentProfileConfig, Config, WebToolsConfig, WeixinConfig};
use sidekick::control::{BootstrapAdmin, ControlStore, Role, RuntimeManager};
use sidekick::mcp::{McpServerInfo, McpServerToolAction, McpToolInfo};
...
async fn login_sets_cookie_and_me_returns_authenticated_user() {
    ...
    let cookie = login
        ...
        .expect("set-cookie")
        .to_string();
    assert!(cookie.starts_with("sidekick_session="), "{cookie}");
...
async fn skills_api_toggles_workspace_state_in_single_user_mode_without_rewriting_skill_body() {
    ...
    assert_eq!(response.status(), StatusCode::OK);
    let state_path = dir.path().join(".sidekick").join("skills-state.json");
...
async fn skills_api_supports_discovered_skill_ids_that_are_not_slug_safe() {
    ...
    let state_json: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(dir.path().join(".sidekick").join("skills-state.json"))
            .expect("state file"),
    )
    .expect("state json");
...
async fn skills_api_deletes_workspace_skills_even_if_state_is_malformed() {
    ...
    std::fs::create_dir_all(dir.path().join(".sidekick")).expect("sidekick dir");
    let malformed = "{not-json";
    std::fs::write(
        dir.path().join(".sidekick").join("skills-state.json"),
        malformed,
    )
...
async fn skills_api_rejects_path_traversal_skill_ids() {
    ...
    assert!(dir.path().join(".sidekick").exists());
}
