...
// ── Storage keys ──────────────────────────────────────────────────────────────
const SESSION_KEY = "sidekick.sessionId";
const LEGACY_SESSION_KEY = "pikachu.sessionId";
const SELECTED_CHANNEL_KEY = "sidekick.selectedChannel";
const LEGACY_SELECTED_CHANNEL_KEY = "pikachu.selectedChannel";
const SELECTED_SESSION_KEY = "sidekick.selectedSessionId";
const LEGACY_SELECTED_SESSION_KEY = "pikachu.selectedSessionId";
const THEME_KEY = "sidekick.theme";
const LEGACY_THEME_KEY = "pikachu.theme";
const COLLAPSED_KEY = "sidekick.sidebarCollapsed";
const LEGACY_COLLAPSED_KEY = "pikachu.sidebarCollapsed";
const WIDE_KEY = "sidekick.wideLayout";
...
const legacyStoredSessionId = localStorage.getItem(LEGACY_SESSION_KEY);
...
function setSelectedSession(channel, sessionId) {
  ...
  if (channel && sessionId) {
    ...
    localStorage.removeItem(LEGACY_SELECTED_CHANNEL_KEY);
    localStorage.removeItem(LEGACY_SELECTED_SESSION_KEY);
...
function legacyDraftKey() {
  return currentChannel && currentSessionId
    ? `${LEGACY_DRAFT_KEY_PREFIX}.${currentChannel}.${currentSessionId}`
    : null;
}

function saveDraft() {
  const key = draftKey();
  const legacyKey = legacyDraftKey();
  if (!key) return;
...
function restoreDraft() {
  ...
  const legacyKey = legacyDraftKey();
  messageInput.value =
    (key && (localStorage.getItem(key) || (legacyKey && localStorage.getItem(legacyKey)))) || "";
  autoResize();
  updateSlashMenu();
}
...
const savedTheme =
  readStored(THEME_KEY, LEGACY_THEME_KEY) ||
...
function applyWide(wide) {
  ...
  localStorage.removeItem(LEGACY_WIDE_KEY);
}

const savedWide = readStored(WIDE_KEY, LEGACY_WIDE_KEY) !== "false";
applyWide(savedWide);
...
setSidebarCollapsed(readStored(COLLAPSED_KEY, LEGACY_COLLAPSED_KEY) === "true");
